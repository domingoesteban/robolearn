...
def plot_sum_specific_costs(gps_directory_names, itr_to_load=None,
    ...
    if plots_type.lower() == 'iteration':
        ...
        #     raise NotImplementedError('I THINK THIS PART IS WRONG')
        #     for cond in range(total_cond):
        #         fig, ax = plt.subplots(1, 1)
        #         fig.subplots_adjust(hspace=0)
        #         if not latex_plot:
        #             fig.suptitle("Policy Sum Specific Costs for condition %02d (over %02d runs)"
        #                          % (cond, max_available_runs),
        #                          fontsize=30, weight='bold')
        #         fig.canvas.set_window_title('Policy Sum Specific Cost Condition %02d'
        #                                     % cond)
        ...
        #             sum_costs = np.zeros((total_runs, total_itr))
        #             for rr in range(total_runs):
        #                 pol_cost_sum = pol_sample_lists_cost_compos[gps][rr][cond, :, :, :, :].sum(axis=3) # Sum over T
        #
        ...
        #
        #             # Mean over runs
        #             mean_sum_costs = np.mean(sum_costs, axis=0)
        #             max_sum_costs = np.max(sum_costs, axis=0)
        #             min_sum_costs = np.min(sum_costs, axis=0)
        #
        ...
        fig.subplots_adjust(hspace=0)
        if not latex_plot:
            fig.suptitle("Policy Sum Specific Costs for training conditions"
                         " (over %02d runs)" % max_available_runs,
                         fontsize=30, weight='bold')
        fig.canvas.set_window_title('Policy Sum Specific Cost')
        fig.set_facecolor((1, 1, 1))
        des_colormap = [colormap(i) for i in np.linspace(0, 1, total_gps)]
        ...
        for gps in range(total_gps):
            ...
            if conds_to_combine is None:
                total_cost_sum = np.sum(runs_avg, axis=0)
            else:
                total_cost_sum = np.sum(runs_avg[conds_to_combine, :], axis=0)
