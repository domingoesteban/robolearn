...
# PAPER
# gps_directory_names = ['normal_gps', 'dual_gps', 'good_gps', 'bad_gps']
# gps_models_labels = ['mdgps','dmdgps', 'gmdgps', 'bmdgps']
gps_directory_names = ['normal_gps', 'normal_gps2', 'normal_gps3', 'normal_gps4']#, 'normal_gps5']
gps_models_labels = ['safe', 'bigger-safe', 'safe+final', 'safe+final+dual']#, 'safe+final+dual+always']
...
options = {
    ...
    'plot_train_errors': False,  # Number of errors
    #'plot_duality_influence': False,
    #'plot_duality_traj_distr': False,
...
for gps, gps_directory_name in enumerate(gps_directory_names):
    gps_path = '/home/desteban/workspace/robolearn/scenarios/dualist_gps/' + gps_directory_name

    max_available_runs = 0
    for rr in range(100):
        if os.path.exists(gps_path + ('/run_%02d' % rr)):
    ...
    iteration_data_list[gps] = [list() for _ in range(max_available_runs)]
    good_duality_info_list[gps] = [list() for _ in range(max_available_runs)]
    good_trajectories_info_list[gps] = [list() for _ in range(max_available_runs)]
    bad_duality_info_list[gps] = [list() for _ in range(max_available_runs)]
    bad_trajectories_info_list[gps] = [list() for _ in range(max_available_runs)]
    ...
    pol_sample_lists_cost_compositions[gps] = [list() for _ in range(max_available_runs)]
    ...
    for rr in range(max_available_runs):
        ...
        print("Iterations to load in %s: %s" % (gps_directory_name,
                                                itr_to_load))
        for pp in itr_to_load:
            if load_iteration_data or load_duality_data or load_policy_different_data:
                file_to_load = gps_path + str('/run_%02d' % rr) + \
                               str('/itr_%02d/' % pp) + \
                               method.upper() + '_iteration_data_itr_' + \
                               str('%02d' % pp)+'.pkl'
                if os.path.isfile(file_to_load):
                    print('Loading GPS iteration_data from iteration %02d' % pp)
                    iter_data = pickle.load(open(file_to_load, 'rb'))
                    iteration_data_list[gps][rr].append(iter_data)
                    iteration_ids[gps][rr].append(pp+1)

            if load_duality_data:
                file_to_load = gps_path + str('/run_%02d/' % rr) + \
                               str('itr_%02d/' % pp) + \
                               'bad_duality_info_itr_'+str('%02d' % pp)+'.pkl'
                if os.path.isfile(file_to_load):
                    print('Loading GPS good_data from iteration %02d' % pp)

                    file_to_load = gps_path + str('/run_%02d/' % rr) + \
                                   str('itr_%02d/' % pp) + \
                                   'bad_duality_info_itr_'+str('%02d' % pp) +\
                                   '.pkl'
                    bad_dual_data = pickle.load(open(file_to_load, 'rb'))
                    bad_duality_info_list[gps][rr].append(bad_dual_data)

                    file_to_load = gps_path + str('/run_%02d/' % rr) + \
                                   str('itr_%02d/' % pp) + \
                                   'good_duality_info_itr_'+str('%02d' % pp) +\
                                   '.pkl'
                    good_dual_data = pickle.load(open(file_to_load, 'rb'))
                    good_duality_info_list[gps][rr].append(good_dual_data)

                    file_to_load = gps_path + str('/run_%02d/' % rr) + \
                                   str('itr_%02d/' % pp) + \
                                   'bad_trajectories_info_itr_' + \
                                   str('%02d' % pp) + '.pkl'
                    bad_traj_data = pickle.load(open(file_to_load, 'rb'))
                    bad_trajectories_info_list[gps][rr].append(bad_traj_data)

                    file_to_load = gps_path + str('/run_%02d/' % rr) + \
                                   str('itr_%02d/' % pp) + \
                                   'good_trajectories_info_itr_' + \
                                   str('%02d' % pp) + '.pkl'
                    good_traj_data = pickle.load(open(file_to_load, 'rb'))
                    good_trajectories_info_list[gps][rr].append(good_traj_data)

            if load_policy_different_data:
                file_to_load = gps_path + str('/run_%02d/' % rr) + \
                               str('itr_%02d/' % pp) + \
                               'pol_sample_cost_itr_' + str('%02d' % pp) + \
                               '.pkl'
                if os.path.isfile(file_to_load):
                    print('Loading policy sample cost from iteration %02d' % pp)
                    pol_cost_data = pickle.load(open(file_to_load, 'rb'))
                    pol_sample_lists_costs[gps][rr].append(pol_cost_data)

                file_to_load = gps_path + str('/run_%02d/' % rr) + \
                               str('itr_%02d/' % pp) + \
                               'pol_sample_cost_composition_itr_' + \
                               str('%02d' % pp)+'.pkl'
                if os.path.isfile(file_to_load):
...
if load_iteration_data:
    ...
    T = iteration_data_list[-1][-1][-1][-1].sample_list.\
        get_actions(samples_idx).shape[1]
...
total_itr = len(data_list_with_data[-1][-1])
total_cond = len(data_list_with_data[-1][-1][-1])
...
if options['plot_eta']:
    for cond in range(total_cond):
        ...
        for gps in range(total_gps):
            ...
            lines[gps] = ax.plot(range(1, total_itr+1), etas,
                                 color=colormap_list[gps],
...
if options['plot_cs']:
    for gps in range(total_gps):
        for cond in range(total_cond):
            ...
            for itr in range(total_itr):
                samples_cost_sum = iteration_data_list[gps][itr][cond].cs.sum(axis=1)
            ...
            ax.fill_between(range(1, total_itr+1), min_costs, max_costs,
                            alpha=0.5, color=cs_color)
            ax.xaxis.set_major_locator(MaxNLocator(integer=True))
...
if options['plot_policy_actions']:
    if total_gps > 1:
        gps = input('Which gps to use? %s | Select index:'
                    % gps_directory_names)
    else:
        gps = -1
    for cond in range(total_cond):
        dData = iteration_data_list[-1][0][cond].\
            pol_info.policy_samples.get_actions(samples_idx).shape[-1]
        fig, axs = plt.subplots(int(math.ceil(float(dData)/sample_list_cols)),
                                sample_list_cols)
        fig.subplots_adjust(hspace=0)
        ...
        for ii in range(axs.size):
            if axs.ndim > 1:
                ...
                ax = axs[ii % sample_list_cols]

            ax.set_prop_cycle('color', [colormap(i) for i in
                                        np.linspace(0, 1, total_itr)])

        lines = list()
        labels = list()
        for itr in range(total_itr):
            actions = iteration_data_list[gps][itr][cond].pol_info.policy_samples.get_actions(samples_idx)
            for ii in range(axs.size):
                if axs.ndim > 1:
                    ax = axs[ii/sample_list_cols, ii % sample_list_cols]
                else:
...
if options['plot_policy_costs']:
    ...
    total_cond = len(pol_sample_lists_costs[-1][-1][-1])
    total_itr = len(pol_sample_lists_costs[-1][-1])

    temp_array = np.zeros(len(iteration_ids[-1][-1]))
    ...
    if plots_type.lower() == 'iteration':
        ...
        for cond in range(total_cond):
            ...
            for gps in range(total_gps):
                max_runs = len(iteration_ids[gps])
                # Shape [runs x itrs]
                mean_costs = np.zeros(len(iteration_ids[gps][-1]))
                max_costs = np.zeros(len(iteration_ids[gps][-1]))
                min_costs = np.zeros(len(iteration_ids[gps][-1]))
                std_costs = np.zeros(len(iteration_ids[gps][-1]))

                for itr in range(len(iteration_ids[gps][-1])):
                    total_samples = len(pol_sample_lists_costs[gps][-1][itr][cond])
                    samples_cost_sum = np.zeros((max_runs, total_samples))
                    for rr in range(max_runs):
                        samples_cost_sum[rr, :] = \
                            pol_sample_lists_costs[gps][rr][itr][cond].sum(axis=1)

                    temp_array[itr] = samples_cost_sum[-1][-1]

                    mean_costs[itr] = np.mean(samples_cost_sum.mean(axis=1))
                    max_costs[itr] = np.max(samples_cost_sum.mean(axis=1))
                    min_costs[itr] = np.min(samples_cost_sum.mean(axis=1))
                    std_costs[itr] = np.std(samples_cost_sum.mean(axis=1))

                    if iteration_ids[gps][0][itr] < min_iteration:
                        min_iteration = iteration_ids[gps][0][itr]

                    if iteration_ids[gps][0][itr] > max_iteration:
                        max_iteration = iteration_ids[gps][0][itr]
            ...
            #ax.set_title('Policy Costs | Condition %d' % cond)
                ...
                if plot_cost_types:
                    ...
                    mean_cost_types = np.zeros([len(iteration_ids[gps][rr]), total_cost_types])
                    max_cost_types = np.zeros([len(iteration_ids[gps][rr]), total_cost_types])
                    min_cost_types = np.zeros([len(iteration_ids[gps][rr]), total_cost_types])
                    std_cost_types = np.zeros([len(iteration_ids[gps][rr]), total_cost_types])
                    for itr in range(total_itr):
                ...
                best_index = np.argpartition(temp_array, len(iteration_ids[gps][rr])-1)[:len(iteration_ids[gps][rr])-1]
                best_index = best_index[np.argsort(temp_array[best_index])]
                worst_index = np.argpartition(temp_array, -(len(iteration_ids[gps][rr])-1))[-(len(iteration_ids[gps][rr])-1):]
                worst_index = worst_index[np.argsort(-temp_array[worst_index])]
                print('best_index:', best_index+1)
                print('%'*10)
