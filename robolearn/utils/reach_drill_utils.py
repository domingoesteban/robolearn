...
def create_bigman_drill_condition(q, bigman_drill_pose, state_info, joint_idxs=None):
    ...
    print('TODO create_bigman_drill_cond')
...
def create_drill_relative_pose(drill_x=0.75, drill_y=0.00, drill_z=0.0184, drill_yaw=0):
    """
    ...
    :return:         
    """
    drill_quat = tf.transformations.quaternion_from_matrix(tf.transformations.rotation_matrix(np.deg2rad(drill_yaw),
                                                                                            [0, 0, 1]))
    #drill_matrix = homogeneous_matrix(rot=drill_orient, pos=drill_position)
    return np.hstack((drill_quat, drill_x, drill_y, drill_z))


def create_hand_relative_pose(drill_pose, hand_x=0.0, hand_y=0.0, hand_z=0.0, hand_yaw=0, hand_pitch=0, hand_roll=0):
    """
    Create Hand Operational point relative pose. Rotation is first yaw, then pitch, and finally roll
    ...
    hand_matrix = hand_matrix.dot(tf.transformations.rotation_matrix(np.deg2rad(-90), [0, 1, 0]))  # Align hand to base
    hand_matrix = hand_matrix.dot(tf.transformations.rotation_matrix(np.deg2rad(hand_yaw), [1, 0, 0]))
    hand_matrix = hand_matrix.dot(tf.transformations.rotation_matrix(np.deg2rad(hand_pitch), [0, 1, 0]))
    hand_matrix = hand_matrix.dot(tf.transformations.rotation_matrix(np.deg2rad(hand_roll), [0, 0, 1]))
...
def reset_condition_bigman_drill_gazebo(condition, state_info):
    ...
    if state_name in state_info['names']:
        bigman_drill_pose = condition[state_info['idx'][state_info['names'].index(state_name)]]
...
class Reset_condition_bigman_drill_gazebo(object):
    ...
    def reset(self, condition):
        if self.bigman_drill_poses:
            ...
            print ('No drill_bigman pose configured in Reset_condition function')


def reset_bigman_drill_gazebo(bigman_drill_pose, drill_size=None):

    #delete_gazebo_model('drill')
    #delete_gazebo_model('drill_support')

    #spawn_drill_gazebo(bigman_drill_pose, drill_size=drill_size)
    set_drill_gazebo_pose(bigman_drill_pose, drill_size=drill_size)
...
def spawn_drill_gazebo(bigman_drill_pose, drill_size=None):
    ...
    #bigman_drill_pose[2] -= drill_size[2]/2.
    ...
    bigman_drill_pose = homogeneous_matrix(pos=bigman_drill_pose[4:],
                                         rot=tf.transformations.quaternion_matrix(bigman_drill_pose[:4]))
...
def generate_reach_joints_trajectories(drill_relative_pose, drill_size, T, q_init, option=0, dt=1, arm='RA'):
    ...
    if arm in ['LA', 'BA']:
        LH_reach_pose = create_hand_relative_pose(drill_relative_pose,
                                                  hand_x=0, hand_y=drill_size[1]/2-0.02, hand_z=0, hand_yaw=0)
    if arm in ['RA', 'BA']:
        RH_reach_pose = create_hand_relative_pose(drill_relative_pose,
                                                  hand_x=0, hand_y=-drill_size[1]/2+0.02, hand_z=0, hand_yaw=0)
...
def generate_lift_joints_trajectories(drill_relative_pose, drill_size, T, q_init, option=0, dt=1):
    # ######## #
    # Lift drill #
    # ######## #
    if option == 0:
