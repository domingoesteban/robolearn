...
class TrajOptLQR(TrajOpt):
    ...
    def __init__(self, T, hyperparams):
        ...
        self.compute_cost_fcn = algorithm.compute_costs
    ...
    def backward(self, prev_traj_distr, traj_info, eta, m, a=None):
        """
        ...
        while fail:
            ...
            if a is None:
                fCm, fcv = self.compute_cost_fcn(m, eta, augment=(not self.cons_per_step))
            else:
                fCm, fcv = self.compute_cost_fcn(a, m, eta, augment=(not self.cons_per_step))
