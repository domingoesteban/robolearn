...
class Joint:
    ...
    def set_position(self, position):
        p.setJointMotorControl2(self.bodies[self.bodyIndex], self.jointIndex,
                                p.POSITION_CONTROL, targetPosition=position)

    def set_velocity(self, velocity):
        p.setJointMotorControl2(self.bodies[self.bodyIndex], self.jointIndex,
                                p.VELOCITY_CONTROL, targetVelocity=velocity)
    ...
    def get_torque(self):
        return p.getJointState(self.bodies[self.bodyIndex], self.jointIndex)[3]
...
class Camera:
    def __init__(self, body, dist=3, width=320, height=320):
        self._cam_body = body
        self._cam_dist = dist
    ...
    def get_image(self):
        ...
        target_pose = p.multiplyTransforms(eye_pose[:3], eye_pose[3:], [0, 0, self._cam_dist], [0, 0, 0, 1])
