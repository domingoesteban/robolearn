...
from robolearn.envs.gz_ros.ros_env_interface import ROSEnvInterface
...
from robolearn.utils.iit.iit_robots_params import centauro_params, bigman_params, centauro_upper_body_params
...
class RobotROSEnvInterface(ROSEnvInterface):
    def __init__(self, robot_name=None, mode='simulation',
                 body_part_active='LA', cmd_type='position',
                 observation_active=None, state_active=None, cmd_freq=100,
                 robot_dyn_model=None,
        ...
        elif robot_name == 'centauro_ub':
            self.robot_params = centauro_upper_body_params
            robot_name = 'centauro'
        elif robot_name == 'bigman':
            self.robot_params = bigman_params
        else:
            raise NotImplementedError("robot %s has not been implemented!"
                                      % robot_name)
        ...
        self.temp_subscriber = \
            rospy.Subscriber("/xbotcore/"+robot_name+"/joint_states", JointStateAdvr,
                             self.temp_state_callback,
                             (self.temp_joint_state_id,
        ...
        self.temp_subscriber = rospy.Subscriber("/optitrack/relative_poses",
                                                RelativePose,
        ...
        for obs_to_activate in observation_active:
            if obs_to_activate['type'] == 'joint_state':
                ...
                self.obs_joint_names = \
                    [self.joint_names[id] for id in obs_to_activate['joints']]
                self.obs_joint_ids = get_indexes_from_list(self.joint_names,
                                                           self.obs_joint_names)
        ...
        # TEMPORAL
        self.temp_publisher = rospy.Publisher("/xbotcore/"+robot_name+"/command",
                                              CommandAdvr,
                                              queue_size=10)
    ...
    def get_env_info(self):
        ...
        :return: Dictionary with obs_info and state_info dictionaries.
                 Each one with keys: names, dimensions and idx.
        """
    ...
    def reset(self, time=None, freq=None, condition=0):
        # Stop First
        self.stop()
        ...
        if condition > len(self.conditions)-1:
            raise AttributeError("Desired condition not available. %d > %d"
                                 % (condition, len(self.conditions)-1))
        ...
        reset_publisher = \
            rospy.Publisher("/xbotcore/"+self.robot_name+"/command",
                            CommandAdvr, queue_size=10)

        joint_positions = get_last_xbot_state_field(self.robot_name,
                                                    'link_position',
                                                    self.act_joint_names)

        final_positions = np.zeros(7)
        if self.robot_name == 'bigman':
            final_positions[1] = np.deg2rad(-90)
        else:
            final_positions[1] = np.deg2rad(90)

        joint_trajectory = polynomial5_interpolation(N*2, final_positions,
                                                     joint_positions)[0]
        ...
        joint_positions = get_last_xbot_state_field(self.robot_name,
                                                    'link_position',
                                                    self.act_joint_names)
        final_positions = self.get_q_from_condition(self.conditions[condition])
        print(joint_positions)
        print(final_positions)
        # input('NOPEEE')
        joint_trajectory = polynomial5_interpolation(N*2,
                                                     final_positions,
                                                     joint_positions)[0]
        print("Moving to condition '%d' in position control mode..." % condition)
        for ii in range(joint_trajectory.shape[0]):
            # reset_cmd.position = joint_trajectory[ii, :]
            reset_cmd.position = final_positions[ii, :]
            reset_publisher.publish(reset_cmd)
            pub_rate.sleep()
        print('TERMINEEEE')


        rospy.sleep(2)  # Because I need to find a good way to reset
        ...
        if self.mode == 'simulation' and self.reset_simulation_fcn is not None:
            ...
            self.reset_simulation_fcn.reset(condition)
    ...
    def add_condition(self, condition):
        if len(condition) != self.state_dim:
            raise AttributeError("Condition and state does not have same "
                                 "dimension!! (%d != %d)"
                                 % (len(condition), self.state_dim))
    ...
    def add_q0(self, q0):
        if len(q0) != len(self.q0[-1]):
            raise ValueError("Desired q0 dimension (%d) does not match with "
                             "Robot (%d)" % (len(q0), len(self.q0[-1])))
        self.q0.append(q0)
    ...
    def temp_state_callback(self, data, params):
        ...
        for idx, name in enumerate(data.name):
            if name in self.robot_params['joints_names']:
                idxs.append(idx)
                ids.append(self.robot_params['joints_names'].index(name))
        joint_ids[:] = ids
        # joint_ids[:] = [self.robot_params['joints_names'].index(name) for name in data.name]
        joint_pos[joint_ids] = [data.link_position[idx] for idx in idxs]
        joint_effort[joint_ids] = [data.effort[idx] for idx in idxs]
        joint_vel[joint_ids] = [data.link_velocity[idx] for idx in idxs]
        joint_stiffness[joint_ids] = [data.stiffness[idx] for idx in idxs]
        joint_damping[joint_ids] = [data.damping[idx] for idx in idxs]
        joint_effort_reference[joint_ids] = [data.effort_reference[idx] for idx in idxs]

    def temp_target_callback(self, data):
        self.receiving_target = True
        tgt_idx = data.name.index(self.temp_object_name)
        self.target_pose[0] = data.pose[tgt_idx].orientation.x
        self.target_pose[1] = data.pose[tgt_idx].orientation.y
        self.target_pose[2] = data.pose[tgt_idx].orientation.z
        self.target_pose[3] = data.pose[tgt_idx].orientation.w
        self.target_pose[4] = data.pose[tgt_idx].position.x
        self.target_pose[5] = data.pose[tgt_idx].position.y
        self.target_pose[6] = data.pose[tgt_idx].position.z
