...
class HIUSACNEW(IncrementalRLAlgorithm, TorchAlgorithm):
    ...
    def __init__(
        ...
        self._i_target_qf2 = self._i_qf2.copy()
    ...
    def _do_training(self):
        ...
        self._i_qf1_optimizer.zero_grad()
        i_qf1_loss.backward()
        self._i_qf1_optimizer.step()
        ...
        if self._auto_alphas:
            ...
            self._u_alpha_optimizer.zero_grad()
            u_alpha_loss.backward()
            self._u_alpha_optimizer.step()

            i_log_alpha = self._i_log_alpha.clamp(max=MAX_LOG_ALPHA)
            i_alpha_loss = -(i_log_alpha * (
                    i_new_log_pi + self._i_tgt_entro).detach()).mean()
            self._i_alpha_optimizer.zero_grad()
            i_alpha_loss.backward()
            self._i_alpha_optimizer.step()
