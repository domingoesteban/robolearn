...
centauro_env = CentauroEnv(interface=interface, mode='simulation',
...
print("\nCentauro body_part_active:%s (action_dim=%d). Command_type:%s" % (body_part_active, action_dim, command_type))
...
try:
    ...
    while episode < total_episodes:
        ...
        for i in xrange(T):
            ...
            obs = np.zeros(5)
            r = 0
            action = np.zeros(centauro_agent.act_dim).reshape([-1, 1])
